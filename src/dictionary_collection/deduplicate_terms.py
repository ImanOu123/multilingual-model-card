...
def remove_term_inflections(term_lst):
    ...
    term_lst = sorted(term_lst, key=lambda x: x['term'])
    ...
    for term_dict in term_lst:
        ...
        if termWOpunct.replace(" ", "") not in tmp_term_lst and len(termWOpunct) > 1:
            ...
            term_dict['processed_term'] = term
            processed_term_lst.append(term_dict)
...
pd.DataFrame().from_records(fin_term_lst).to_csv("growing_dict/final_terms_with_original.csv")